...
final class CamelHealthRuntimeHandler extends AbstractRuntimeOnlyHandler {
    ...
    private static final String FIELD_CONTEXT_STATUS = "status";
    ...
    private void handleCamelHealthOperation(OperationContext context, ModelNode operation) {
        ...
        ServiceController<MutableCamelContextRegistry> controller = (ServiceController<MutableCamelContextRegistry>) serviceRegistry.getService(CamelConstants.CAMEL_CONTEXT_REGISTRY_SERVICE_NAME);
        if (controller == null) {
            healthState = CamelHealthState.UNHEALTHY;
        }

        MutableCamelContextRegistry contextRegistry = controller.getValue();
        if (context == null) {
            healthState = CamelHealthState.UNHEALTHY;
        }
        ...
        if (healthState.equals(CamelHealthState.UNHEALTHY)) {
            result.get(FIELD_CONTEXT_STATUS).set(healthState.name());
        ...
        // No camel contexts, so don't return any results
        if (contexts.isEmpty()) {
            context.completeStep(OperationContext.RollbackHandler.NOOP_ROLLBACK_HANDLER);
            return;
        }
        ...
        for (CamelContext camelctx : contexts) {
            ...
            contextHealthNode.get().set(FIELD_CONTEXT_STATUS, contextHealthState.name());
