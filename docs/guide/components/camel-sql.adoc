...
component allows you to work with databases using https://github.com/apache/camel/blob/camel-{camel-version}/components/camel-jdbc/src/main/docs/jdbc-component.adoc[JDBC,window=_blank] queries. The difference between this component and JDBC component is that in case of SQL the query is a property of the endpoint and it uses message payload as parameters passed to the query.
